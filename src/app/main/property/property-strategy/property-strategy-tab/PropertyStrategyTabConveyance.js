...
import {
  getPropertyStrategyConveyanceForm,
...
const PropertyStrategyTabConveyance = () => {
  ...
  const formDef = useSelector(
    ({ property }) => property.strategy.conveyanceForm
  );
  ...
  const stateAction = useSelector(
    ({ property }) => property.strategy.conveyanceAction
  );
  ...
  useEffect(() => {
    if (
      hasAccess(
        contextResources,
        resourceKeys.Property_Details_Tab_Strategy_Conveyance_Tab_API_CALL
      )
    ) {
      dispatch(getProjectionsListAction(caseInstanceId));
      dispatch(getPropertyStrategyConveyanceForm());
    }
  ...
  const onNavButtonHandler = () => {
    document
      .querySelector(
        `a[href='/property/details/${caseInstanceId}/strategy/cwcot']`
      )
      .click();
  };
  ...
  return (
    <div className={clsx('w-full p-12', classes.root)}>
      <div className={clsx('flex sm:flex-row', classes.button)}>
        {hasAccess(
          contextResources,
          'Property_Details_Tab_Strategy_Summary_Conveyance_Save'
        ) && (
          <Button
            className="mr-12"
...
export default PropertyStrategyTabConveyance;
