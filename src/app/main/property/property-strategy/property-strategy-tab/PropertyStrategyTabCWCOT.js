...
const PropertyStrategyTabCWCOT = () => {
  ...
  const formDef = useSelector(({ property }) => property.strategy.cwcotForm);
  const [formButton, setformButton] = useState(false);

  const stateAction = useSelector(
    ({ property }) => property.strategy.cwcotAction
  );
  ...
  useEffect(() => {
    if (
      hasAccess(
        contextResources,
        resourceKeys.Property_Details_Tab_Strategy_Cwcot_Tab_API_CALL
      )
    ) {
      dispatch(getProjectionsListAction(caseInstanceId));
      dispatch(getPropertyStrategyCWCOTForm());
    }
  }, [dispatch, contextResources, caseInstanceId]);
  ...
  const onNavButtonHandler = () => {
    document
      .querySelector(
        `a[href='/property/details/${caseInstanceId}/strategy/reo-asis']`
      )
      .click();
  };
  ...
  return (
    <div className={clsx('w-full p-12', classes.root)}>
      <div className={clsx('flex sm:flex-row', classes.button)}>
        {hasAccess(
          contextResources,
          'Property_Details_Tab_Strategy_Summary_CWCOT_Save'
        ) && (
          <Button
            className="mr-12"
