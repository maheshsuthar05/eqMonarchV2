...
import {
  getPropertyStrategyReoRepairForm,
...
const PropertyStrategyTabReoRepair = () => {
  ...
  const formDef = useSelector(
    ({ property }) => property.strategy.reoRepairForm
  );
  const stateAction = useSelector(
    ({ property }) => property.strategy.reoRepairAction
  );
  ...
  useEffect(() => {
    if (
      hasAccess(
        contextResources,
        resourceKeys.Property_Details_Tab_Strategy_ReoRepair_Tab_API_CALL
      )
    ) {
      dispatch(getProjectionsListAction(caseInstanceId));
      dispatch(getPropertyStrategyReoRepairForm());
    }
  ...
  return (
    <div className={clsx('w-full p-12', classes.root)}>
      <div className={clsx('flex sm:flex-row', classes.button)}>
        {hasAccess(
          contextResources,
          'Property_Details_Tab_Strategy_Summary_REORepair_Save'
        ) && (
          <Button
            className="mr-12"
...
export default PropertyStrategyTabReoRepair;
